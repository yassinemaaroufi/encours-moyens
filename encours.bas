...
'Extraction des num�ros d'op�ration des remboursements lin�aires
With ESH
Dim arr As Variant
i = 2
codes = .Cells(i, IEN)
...
Do Until IsEmpty(.Cells(i, 3))
    ...
    If .Cells(i, IPBP) <> "EMLT" Then
        ...
        Next j
        If lin = 1 Then
        'Remboursement lin�aire (traitement � part)
            k = 2
...
'''' D�but du calcul des encours moyens ''''''''''''
'affiche_titres_encours_op_prets psh, iec, "PRETS"
affiche_titres_encours_op PSH, IEC, "PRETS"
dd = o.Item(str(0)).Item("date")      'Date de la derni�re op�ration
k = 0
